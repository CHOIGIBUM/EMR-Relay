...
export default function Brand({ subtitle, mobile = false, onHome }: { subtitle: string; mobile?: boolean; onHome?: () => void }) {
  const auth = useAuth();
  return (
    <header className={mobile ? styles.mobileHeader : styles.desktopHeader}>
      ...
      <div className={styles.headerRight}><button type="button" className={styles.logout} onClick={auth.signOut} aria-label="로그아웃"><LogOut size={17} /><span>로그아웃</span></button></div>
    </header>
  );
}
