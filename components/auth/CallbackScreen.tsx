...
export default function CallbackScreen() {
  ...
  return <main className={styles.page}><section className={styles.card}><AuthBrand /><div className={styles.status}>{error ?? "로그인을 완료하고 있습니다."}</div>{error && <button className={styles.linkButton} onClick={() => window.location.replace("/login")}>로그인으로 돌아가기</button>}</section></main>;
}
