...
test("renders labeled route endpoints, range circles, expansion controls, and acceptance alerts", () => {
  ...
  assert.match(matchMapSource, /map\.setBounds\(bounds\)/);
  assert.match(matchMapSource, /현재 요청 반경 \{radiusKm\}km/);
  assert.match(matchMapSource, /radiusKm === 0 \? " · 실행 전"/);
  assert.match(matchMapSource, /다음 \$\{nextRadiusKm\}km 확대 가능/);
  assert.match(matchMapSource, /현재 \$\{radiusKm\}km 요청 완료 · 다음 \$\{nextRadiusKm\}km/);
  assert.doesNotMatch(matchMapSource, /30초 후 자동 확대/);
  assert.match(matchMapSource, /현재 요청: \{expansionReasonLabel\[expansionReason\]\}/);
  assert.match(matchMapSource, /요청 범위 확대 버튼을 누를 때마다 15 → 30 → 60 → 120km로 한 단계씩 확대/);
  ...
  assert.doesNotMatch(paramedicSource, /nextExpansionAt=\{matchingState\?\.nextExpansionAt\}/);
